        ...
        <?php if ($sf_user->isAuthenticated()): ?>
            <div style="margin:-5px 0 0 -5px; padding:0 0 5px 0; overflow:hidden; //zoom:1;">
                <?php if($photo): ?>
                    ...
                    <?php endif; ?>
                    <?php if($cnt_photo>5): ?>
                    <div style="display: none;"><?= link_to(image_tag('/uploads/photo/small_thumbnail/'.$ph->getImage(),array('class'=>'middle_inner')),'http://'.$_SERVER['HTTP_HOST'].'/uploads/photo/original/'.$ph->getImage(),array('class'=>'popup','rel'=>"group1")) ?></div>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
         <?php endif; ?>
...
<div style="overflow:hidden; //zoom:1;">
    <h1 align="center" style="margin-left:65px; position:relative; padding-right:65px;">
        <img src="/pic/t_left.png" style="position:absolute; left:-67px; bottom:0;"><?= $p->getFirstName() ?> <?= $p->getLastName() ?></h1>
    ...
    <div class="icon_pre_wrap2">
        ...
        <div class="icon_pre_item">
            <a title="<?= __('Call to chat') ?>" onClick="window.open('<?= url_for('chat/dispatcher?user_id='.$p->getUserId()) ?>','chatwindow','width=700,height=600')" href="#" ><img src="/pic/<?= $imc ?>"></a>
        </div>
        ...
        -->
        <div class="icon_pre_item">
            <a href="<?= url_for('message/send?username='.$p->getsfGuardUser()->getUsername()) ?>" title="<?= __('Send message') ?>"><img src="/pic/icon_nav_2.png"></a>
        </div>

        <div class="icon_pre_item">
            <a href="<?= url_for('interest/add?username='.$p->getsfGuardUser()->getUsername())?>" title="<?= __('Send interest') ?>"><img src="/pic/icon_nav_4.png"></a>
        </div>


        <div class="icon_pre_item">
            <a href="<?= url_for('friend/add?username='.$p->getsfGuardUser()->getUsername()) ?>" title="<?= __('Add to favorites') ?>"><img src="/pic/icon_nav_1.png"></a>
        </div>
