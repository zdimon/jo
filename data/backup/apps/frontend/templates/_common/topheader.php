<?php

if($sf_user->isAuthenticated())
{
   if($sf_user->getGuardUser()->getGender()=='m')
   {
       $gen = 'w';
   }
    else
    {
        $gen = 'm';
    }
}
else
{
    $gen = 'w';
}

  $q= Doctrine::getTable('Profile')
->createQuery('a')
->orderBy('a.rating DESC')
->leftJoin('a.sfGuardUser u')
->where('a.status_id=? and a.gender=?',array(1,$gen))
->limit(7);
  if ($sf_user->isAuthenticated())
{
    //$q->addWhere('a.gender=?',array($sf_user->getGuardUser()->getAntiGender()));
}
  $items = $q->execute();
  $data = '<ul id="mycarousel" class="jcarousel-skin-tango">';
  foreach($items as $i)
  {
      $arr = array (__('Visit my profile'),__('Write to me'),__('Contact me'));
      $data.= '<li style="float: left;"><a title="'.$i->getsfGuardUser()->getProfile()->getImgSeo().'" href="'.url_for('profile/show?username='.$i->getsfGuardUser()->getUsername()).'"><span class="joinMeBule">'.$arr[rand(0,2)].'</span>'.image_tag($i->getUrlImageMiddle()).'</a></li>';
  }
  $data .= '</ul>';

echo $data;
?>




       