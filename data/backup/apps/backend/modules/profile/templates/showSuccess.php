...
<?php endforeach ?>
    <div style="clear: both"></div>
<?php endif ?>
<p><?= link_to(__('+ фото'),'profile/addphoto?id='.$p->getUserId(),array('class'=>'ui-state-default ui-corner-all')) ?></p>
<br />
...
<p><?= link_to(__('+ видео'),'profile/addvideo?id='.$p->getUserId(),array('class'=>'ui-state-default ui-corner-all')) ?></p>
<br />

<?php if(sfConfig::get('app_menu_partner')!='false' and $sf_user->hasCredential('admin')):?>
  <p><a style="display: block; width: 170px" href="<?= url_for('task/new?user_id='.$p->getUserId()) ?>" id="dialog_link" class="ui-state-default ui-corner-all"><span class="ui-icon ui-icon-copy"></span><?= __('Выдать задание партнеру') ?></a></p>
<?php endif ?>
