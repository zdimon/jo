...
<form method="post" action="<?php echo url_for('@sf_guard_signin') ?>">
    ...
    <div class="pb25 italic bold">
        <label class="sUser_label">
            <?= __('Login') ?>
    </label>
        <input  class="sUser_i"  type="text" name="signin[username]"  value="" />
        <?= $formi['username']->renderError() ?></div>
    <div class="pb25 italic bold"><label class="sUser_label"><?= __('Password') ?></label> <input  class="sUser_i"  type="password" name="signin[password]" value="" />
        <?= $formi['password']->renderError() ?></div>
